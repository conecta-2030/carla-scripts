...
class World(object):
    ...
    def restart(self, weather_preset=None):
        ...
        if self._weather_preset and self._weather_preset in WEATHER_PRESETS:
            print(f"Applying weather preset: {weather_preset}")
            self.world.set_weather(WEATHER_PRESETS[self._weather_preset])
        else:
...
class CollisionSensor(object):
    ...
    def _on_collision(weak_self, event):
        ...
        self.crashSound = pygame.mixer.Sound('scream.mp3')

def _on_collision(weak_self, event):
    # Certifique-se de que weak_self ainda existe
    self = weak_self()
    if not self:
        return
    
    # Verifica se o outro ator é um pedestre
